...
The project uses a **custom 2-step Gemini pipeline** (not the formal CrewAI `Agent`/`Task`/`Crew` classes). The backend calls Google Gemini 1.5 Flash directly via its REST API, with two sequential prompts.
...
| **API** | Direct REST: `POST /v1beta/models/gemini-flash-latest:generateContent` |
